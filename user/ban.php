<?php
	...
	$stmt = $conn->prepare("UPDATE accounts SET permissions = 0 WHERE id = ?");
	...
	//Remove the packs from the database. This will cascade versions, tags, and favorites,
	$stmt = $conn->prepare("DELETE FROM packs WHERE account_id = ?");
	$stmt->bind_param("i", $targetID);
	$stmt->execute();
